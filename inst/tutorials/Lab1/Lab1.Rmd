---
title: "Crash Course"
description: "Your first baby steps into R"
runtime: shiny_prerendered
output:
  learnr::tutorial:
    progressive: true
    allow_skip: false
    id: "ff0ef91f-7b5b-4f22-87ed-8eb9583254f2"
    version: 2
    mathjax: default
    toc: true
editor_options:
...
#library(praise)
library(utils)
knitr::opts_chunk$set(echo = TRUE, tidy = T, root.dir = '/Users/bbitarello/Documents/GitHub/DataSetsb215/inst/tutorials/Lab1/')
```
...
In your prompt, type `?log` or `help(log)`:
...
```{r,  sandbox-help-log, echo = T, exercise=TRUE, eval = FALSE }
...
certain the function exists, you can try `??tstrsplit`:

```{r, echo = T, eval = F, include = F}
??tstrsplit
```

This will show any match to "tstrsplit" within installed packages in your workspace. Something like this:
![](images/image3.png)
...
```{r sandbox-quiz, exercise=F, echo = T, eval = F}

learnr::run_tutorial("Quiz1.Rmd")

```


```{r download-answers, echo =F,child = system.file("child_documents/download_answers.Rmd", package = "tutorial.helpers")}
```
## Links & References
